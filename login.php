...
if(isset($_POST['email']) && isset($_POST['passwort'])) {
	...
	if ($user !== false && password_verify($passwort, $user['passwort'])) {
		...
		if(isset($_POST['angemeldet_bleiben'])) {
			...
			setcookie("identifier",$identifier,time()+(3600*30)); //Valid for 30 days
			setcookie("securitytoken",$securitytoken,time()+(3600*30)); //Valid for 30 days
		}

		header("location: mitarbeiterliste.php");
		exit;
	} else {
